 ...
 *
 * @note
 * 对于结构矩形三角网格，create_structured_rectangle() 生成的每个三角形都满足：
 *
 * - 节点按逆时针方向排列
 * - 直角点放在 node_ids[1]，即中间节点位置
 */
...
public:
     ...
     *
     * @note
     * 对非结构网格，本函数会尽量保证三角形在 XY 平面投影下为逆时针。
     * 如果三角形在 XY 平面退化，则保留用户输入顺序。
     */
