...
//     // 创建 Ribbon Page
//     FilePage* filePage = new FilePage(ribbon, logWidget, this);
//     filePage->create();
//     connect(filePage, &FilePage::importModelTriggered, this, &MainWindow::onImportModel);
//     connect(filePage, &FilePage::exportModelTriggered, this, &MainWindow::onExportModel);

//     ViewPage* viewPage = new ViewPage(ribbon, logWidget, this);
//     viewPage->create();
// }
...
void MainWindow::initCentralVTKView()
{
    ...
    QWidget* inner = new QWidget(vtkFrame);
...
void MainWindow::initDockWidgets()
{
    // -------- Tree --------
    dockTree = new QDockWidget(tr("模型树"), this);
    QWidget* treeBg = new QWidget(dockTree);
    treeBg->setStyleSheet("background:#D0D0D0;");
    QVBoxLayout* treeBgLayout = new QVBoxLayout(treeBg);
    treeBgLayout->setContentsMargins(4,4,4,4);

    treeWidget = new QTreeWidget(treeBg);
    treeWidget->setHeaderLabel("Model Explorer");
    treeBgLayout->addWidget(treeWidget);

    dockTree->setWidget(treeBg);
    addDockWidget(Qt::LeftDockWidgetArea, dockTree);

    // -------- Log --------
    dockLog = new QDockWidget(tr("日志"), this);
    QWidget* logBg = new QWidget(dockLog);
    logBg->setStyleSheet("background:#D0D0D0;");
    QVBoxLayout* logBgLayout = new QVBoxLayout(logBg);
    logBgLayout->setContentsMargins(4,4,4,4);

    logWidget = new QTextEdit(logBg);
    logWidget->setReadOnly(true);
    logWidget->append("[INFO] OpenFinite started.");

    logBgLayout->addWidget(logWidget);
    dockLog->setWidget(logBg);
    addDockWidget(Qt::BottomDockWidgetArea, dockLog);
}
...
void MainWindow::initRibbon()
{
    ...
    filePage->create();
     ...
     // // ===== 视图 Page =====
    ViewPage* viewPage = new ViewPage(ribbon, wid_vtk, logWidget, this);
    viewPage->create();

    // // ===== 几何 Page =====
    auto* geomPage = new GeometryPage(ribbon, logWidget, this);
    geomPage->create();

    // // ===== 网格 Page =====
    auto* meshPage = new MeshPage(ribbon, logWidget, this);
    meshPage->create();

    // // ===== 求解 Page =====
    auto* solvePage = new SolvePage(ribbon, logWidget, this);
    solvePage->create();

    // ===== 后处理 Page =====
    auto* postPage = new PostPage(ribbon, logWidget, this);
    postPage->create();
}
