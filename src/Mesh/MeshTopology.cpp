...
)
{
    ...
    std::vector<std::set<int>> node2edge_sets(num_nodes);
    std::vector<std::set<int>> node2node_sets(num_nodes);
    std::vector<std::set<int>> cell2cell_sets(num_cells);
    ...
    for (int cid = 0; cid < num_cells; ++cid) {
        ...
        for (const auto& e : local) {
            ...
            node2edge_sets[ce[0]].insert(eid);
            node2edge_sets[ce[1]].insert(eid);

            node2node_sets[ce[0]].insert(ce[1]);
            node2node_sets[ce[1]].insert(ce[0]);
        }
    }

    for (int nid = 0; nid < num_nodes; ++nid) {
        node2edge_[nid].assign(
            node2edge_sets[nid].begin(),
            node2edge_sets[nid].end()
        );

        node2node_[nid].assign(
            node2node_sets[nid].begin(),
            node2node_sets[nid].end()
        );
    }
    ...
    for (int eid = 0; eid < static_cast<int>(edge2cell_.size()); ++eid) {
        const auto& adjacent_cells = edge2cell_[eid];
        ...
        if (adjacent_cells.size() == 1) {
            ...
            boundary_cell_set.insert(adjacent_cells[0]);
        }

        for (std::size_t i = 0; i < adjacent_cells.size(); ++i) {
            for (std::size_t j = 0; j < adjacent_cells.size(); ++j) {
                if (i == j) {
                    continue;
                }

                cell2cell_sets[adjacent_cells[i]].insert(
                    adjacent_cells[j]
                );
            }
        }
    ...
    for (int cid = 0; cid < num_cells; ++cid) {
        cell2cell_[cid].assign(
            cell2cell_sets[cid].begin(),
            cell2cell_sets[cid].end()
        );
    }
